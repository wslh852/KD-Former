...
class generate_train_data(Dataset):
    ...
    def __getitem__(self, index):
        ...
        encoder_inputs = data_sel[0:self._source_seq_len ]
        decoder_target = data_sel[self._source_seq_len:]
...
def get_batch_srnn(data, action, source_seq_len, target_seq_len, input_size):
    ...
    decoder_target = np.zeros( (batch_size, target_seq_len, input_size), dtype=float )
